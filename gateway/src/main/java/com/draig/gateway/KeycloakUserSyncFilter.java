package com.draig.gateway;

import com.draig.gateway.user.RegisterRequest;
import com.draig.gateway.user.UserService;
...
public class KeycloakUserSyncFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        // If there's no token, just continue the chain without attempting sync
    ...
    private RegisterRequest getUserDetails(String token) {
        try {
            ...
            log.warn("Failed to parse JWT for user sync: {}", e.getMessage());
            return null;
        }
    }
}